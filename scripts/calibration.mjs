...
try {
  const { CALIBRATION_STUDIES } = await server.ssrLoadModule('/src/print/calibration.ts');
  ...
  const outputs = [];
  for (const study of CALIBRATION_STUDIES) {
    ...
    const project = { format: 'np3dp-project', schemaVersion: 1, recipe: study.recipe, setup };
    outputs.push({ study, result, report, project, compileMs });
  }
  // Validate every study before writing any delivery files.
  await mkdir('examples/calibration', { recursive: true });
  await mkdir('artifacts/session-002', { recursive: true });
  ...
  for (const { study, result, report, project, compileMs } of outputs) {
    ...
    await writeFile(`examples/calibration/${filename}.np3dp-project.json`, serializeProject(project));
    await writeFile(`artifacts/session-002/${filename}.gcode`, result.text);
    await writeFile(`artifacts/session-002/${filename}.print-report.json`, `${JSON.stringify(report, null, 2)}\n`);
  ...
  await writeFile('docs/evidence/session-002-calibration.json', `${JSON.stringify(manifest, null, 2)}\n`);
