...
export function PrintExportDialog({ project, close }: { project: PrintProject; close: () => void }) {
  ...
  return <dialog className="export-dialog print-export-dialog" ref={dialog} onCancel={close} onClick={(event) => { if (event.target === event.currentTarget) close(); }}>
    ...
    <div className="first-print-note"><strong>Print checklist</strong><p>Use the calibrated sheet and first-layer Z setting. Clear the bed and front purge strip, load the recorded PLA, and start from USB. Expect a preview and time estimate, mesh probing, then a two-part purge at the front. The second purge segment should be continuous. Observe the foundation, fan and first patterned turns. For the next batch, print 07 miniature B, then 08 arches, then 09 held spans, inspecting each result before advancing.</p><p>First-layer inset: {project.setup.foundation.elephantFootMm.toFixed(2)} mm. LCD previews show nominal deposition; remaining time uses commanded feed and dwell, excluding thermal waits, homing, probing and firmware dynamics.</p></div>
