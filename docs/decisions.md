...
## Logged engineering defaults — session 005

| ID | Default taken | Reason and practical limit |
|---|---|---|
| E30 | New 07 derives from B: 34 → 38 mm, 24 mm wall, belly 1 mm; same pitch/amplitude/speed. 08/09 use 32 × 10.2 mm walls and a 4.2 mm collar. | Separate shape transfer from new methods; retain 01–06. Height 10.2 mm stays inside the existing schema range and gives whole turns/motifs. These jobs are unprinted. |
| E31 | 08: 0.20 mm staggered arches, no rim. 09: zero-lift, 4.18 mm chords, 0.02 mm³ deposits, 0.10 s holds; rim retained. Share study-specific foundation settings between app and batch. | A rim approached the final arches to ~0.0065 mm, so omit it in this test. Zero-lift separates timed endpoints from the old bridge's double-extruded posts. 09 is nominally supported, not a free-space sag validation. Lifted methods remain editable with explicit diagnostics. |
| E32 | Shared phase-boundary indexing uses eight scaled machine epsilons to suppress numerical duplicate endpoints; wall engine 0.2.0, prepared engine 0.4.0. | Corrects a ghost stationary action at a zero-motion interval. Preflight/emission agree, genuine partial motifs remain, and old recipe meanings/schema are preserved. Exact old G-code remains the historical replay record. |
| E33 | Add emitted-chord matched-revolution XYZ separation with monotone polar-ray matching, bounded samples/scans and applicability reasons. | Includes shape drift and actual method chords; stationary volumes excluded but positions checked. Per-turn angular coverage is not contact probability, nozzle clearance or physics. No numerical physical-accuracy commitment is implied. |
| E34 | Publish the three reviewed G-code/project/report sets directly with the static app, plus a checksum manifest. Generation is explicit via `npm run calibration:publish`; CI recompiles and checks exact delivered bytes. | Makes the next test batch directly downloadable. This extends the earlier choice to keep generated files local for a small reviewable reference batch; arbitrary user exports remain local. Machine files bypass Git line-ending conversion. No new service, dependency or licensing choice. |
